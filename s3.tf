resource "aws_s3_bucket" "nomad_snapshots" {
  bucket           = "${var.project_name}-nomad-snapshots-${data.aws_caller_identity.current.account_id}-${data.aws_region.current.region}-an"
  bucket_namespace = "account-regional"
  force_destroy    = true

  tags = merge(var.common_tags, { Name = "${var.project_name}-nomad-snapshots" })
}

resource "aws_s3_bucket_versioning" "nomad_snapshots" {
