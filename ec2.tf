...
resource "aws_instance" "nomad" {
  ...
  user_data = templatefile("${path.module}/templates/cloud-init.sh.tftpl", {
    ...
    nomad_snapshot_bucket        = aws_s3_bucket.nomad_snapshots.id
    nomad_snapshot_interval      = var.nomad_snapshot_interval
    nomad_snapshot_retain        = var.nomad_snapshot_retain
  })
